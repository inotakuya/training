...
ベクトルまたは行列の中に含まれる数のことを要素と呼び、m×1m×1 の列ベクトル bb の上から ii 番目の要素は bi(i=1,2,...,m)bi(i=1,2,...,m)、
n×mn×m の行列 AA には n×mn×m 個の要素が含まれ、ii 行 jj 列目の要素は aij(i=1,2,...,n,j=1,2,...,m)aij(i=1,2,...,n,j=1,2,...,m)で示されます。
...
ci=∑j=1maijbj=ai1b1+ai2b2+...+aimbm
Input
１行目に nn と mm が空白区切りで与えられます。続く nn 行に行列 AA の要素 aijaij が空白区切りで与えられます。
続く mm 行にベクトル bb の要素 bibi がそれぞれ１行に与えられます。
